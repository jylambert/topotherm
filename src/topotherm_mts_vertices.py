...
def topotherm_mts_vertices(name, time_steps, file_path, result_path, variant, mode):
    ...
    model.P_loss_1 = pyo.Var(model.set_n_i, model.set_t, bounds=(0, p_max_pipe_const), domain=pyo.NonNegativeReals, initialize=0)     # Heat power at the exit of the pipe
    model.P_loss_2 = pyo.Var(model.set_n_i, model.set_t, bounds=(0, p_max_pipe_const), domain=pyo.NonNegativeReals, initialize=0)     # Heat power at the exit of the pipe
    ...
    model.cons_calculation_loss_1_1 = pyo.Constraint(model.set_n_i, model.set_t, rule=calculation_loss_1_1)

    def calculation_loss_1_2(m, j, t):
        return (regression_heat_loss_a * p_max_part_load[0] * m.P_cap[j] + regression_heat_loss_b * m.lambda_dir_1[j, t]) * l_i[j] - m.P_loss_1[j, t] - p_max_pipe_const * (1-m.lambda_dir_1[j, t]) <= 0
    model.cons_calculation_loss_1_2 = pyo.Constraint(model.set_n_i, model.set_t, rule=calculation_loss_1_2)
    ...
    model.cons_calculation_loss_2_1 = pyo.Constraint(model.set_n_i, model.set_t, rule=calculation_loss_2_1)

    def calculation_loss_2_2(m, j, t):
        return (regression_heat_loss_a * p_max_part_load[0] * m.P_cap[j] + regression_heat_loss_b * m.lambda_dir_2[j, t]) * l_i[j] - m.P_loss_2[j, t] - p_max_pipe_const * (1-m.lambda_dir_2[j, t]) <= 0
    model.cons_calculation_loss_2_2 = pyo.Constraint(model.set_n_i, model.set_t, rule=calculation_loss_2_2)
    ...
    # The house connection has to be built to satisfy the demand ij
