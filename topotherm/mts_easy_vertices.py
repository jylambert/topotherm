...
def topotherm_mts_easy_vertices(name, time_steps, file_path, result_path, variant, mode):
    ...
    _, regression_heat_loss = precalc.calc_regression_heat_losses(m_max, supply_temp, np.ones(settings.Piping.number_diameter), 100*np.ones(settings.Piping.number_diameter), temp_ambient, p_max)
    ...
    # Define variables
    dpipes = dict(bounds=(0, p_max_pipe_const), domain=pyo.NonNegativeReals, 
        	initialize=p_max_pipe_const)
    model.P_11 = pyo.Var(model.set_n_i, model.set_t, **dpipes)         # Heat power at the entry of the pipe
    model.P_12 = pyo.Var(model.set_n_i, model.set_t, **dpipes)         # Heat power at the exit of the pipe
    model.P_cap = pyo.Var(model.set_n_i, **dpipes)                     # Thermal capacity of the pipe
    model.P_21 = pyo.Var(model.set_n_i, model.set_t, **dpipes)         # Heat power at the entry of the pipe
    model.P_22 = pyo.Var(model.set_n_i, model.set_t, **dpipes)         # Heat power at the exit of the pipe
