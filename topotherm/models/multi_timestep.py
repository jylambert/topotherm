...
def create(
    ...
    mdl.set_t = pyo.Set(initialize=range(matrices["q_c"].shape[1]), doc="Time steps")
    ...
    mdl.P_cap = pyo.Var(mdl.set_n_i, doc="Thermal capacity of each pipe", **pipe_power)
    ...
    mdl.lambda_b = pyo.Var(
        mdl.set_n_i, initialize=1, domain=pyo.Binary, doc="Binary building decision"
    )
    ...
    def nodal_power_balance(m, j, t):
        ...
        if optimization_mode == "forced":
            ...
            sink = sum(m.lambda_b[k] for k in sets["a_c_out_edge"][j]) * sum(
                matrices["q_c"][k, t] for k in sets["a_c_out"][j]
            )
    ...
    mdl.cons_power_max_P_built_const = pyo.Constraint(
        mdl.set_n_i,
        rule=power_max_p_built_const,
        doc="Maximum Powerflow constant i->j / j->i",
    )
    ...
    mdl.one_pipe = pyo.Constraint(
        mdl.set_n_i, mdl.set_t, rule=one_pipe, doc="Just one Direction for each pipe"
    )
    ...
    if (optimization_mode == "economic") | (optimization_mode == "sensitivity"):
        ...
        mdl.cons_connection_to_consumer = pyo.Constraint(
            mdl.cons, mdl.set_t, rule=connection_to_consumer_eco, doc=msg_
        )
        ...
        msg_ = """Constraint if houses have their own connection-pipe
            and set the direction (ij or ji)"""
        mdl.cons_connection_to_consumer = pyo.Constraint(
            mdl.cons, mdl.set_t, rule=connection_to_consumer_fcd, doc=msg_
        )
        ...
        def total_energy_conservation(m, t):
            return (
                sum(m.P_source[k, t] for k in m.set_n_p)
                - sum(m.P["ij", "in", k, t] - m.P["ij", "out", k, t] for k in m.set_n_i)
                - sum(m.P["ji", "in", k, t] - m.P["ji", "out", k, t] for k in m.set_n_i)
                - sum(matrices["q_c"][k, t] for k in m.set_n_c)
                == 0
            )

        mdl.cons_total_energy_cons = pyo.Constraint(
            mdl.set_t, rule=total_energy_conservation, doc="Total energy conservation"
        )
    ...
    mdl.capex_pipe_constr = pyo.Constraint(
        expr=mdl.capex_pipes == pipes, doc="CAPEX Pipe constraint"
    )
