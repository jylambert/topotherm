...
OUTPUTPATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "results", "sts_div"
)
...
def main(filepath, outputpath, plots=True, solver="gurobi", mode="economic"):
    ...
    # calculate simultaneity factors
    network_simultaneity = tt.simultaneity.calculate(network)

    node_data, edge_data = tt.simultaneity.update_data(
        df_nodes=node_data, df_edges=edge_data, network=network_simultaneity
    )

    opt_mats_div = opt_mats.copy()
    opt_mats_div["p_sim"] = edge_data["power_simultaneity"].values
    opt_mats_div["m_i_0"], opt_mats_div["d_i_opt"], _ = (
        tt.postprocessing.calculate_hydraulics(opt_mats_div["p_sim"], settings)
    )

    if plots:
        f = tt.plotting.district(
            opt_mats, diameter=opt_mats_div["d_i_opt"], isnot_init=True
        )
        f.savefig(
            os.path.join(outputpath, "district_optimal_div.svg"), bbox_inches="tight"
        )
